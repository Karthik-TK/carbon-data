...
const FormUI: React.FC = () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      const fileSize: number =
        Math.round((selectedFile.size * 1000) / Math.pow(1024, 2)) / 1000;
      ...
      if (fileSize > fileSizeLimit) {
        ...
        toast.error("Error: Please upload file size of less than 5 MB");
      } else {
        ...
        reader.onload = (e) => {
          ...
          // Check if the file has correct headers and format
  ...
  const setDisplayTableView = useCallback((value: any) => {
    setFile(null);
    setData([]);
    setShowTableView(false);
  }, []);
        ...
        <div className="container mx-auto p-4">
          ...
          {data.length > 0 && (
            <div className="overflow-y-auto h-96 mt-4 p-4 border border-gray-200 rounded-xl bg-gray-100 shadow-lg">
              <>
                {/* Disabled the save changes button as it requires backend to process the output file */}
                <button
                  className="text-white bg-gray-700 text-sm font-bold rounded-md px-4 py-2 flex justify-center items-center mb-4 cursor-not-allowed hover:bg-gray-500"
                  disabled
